  ...
  private handleConnection(socket: net.Socket) {
    ...
    const onPublisherData = (chunk: Buffer) => {
      ...
      while (offset < chunk.length) {
        if (chunk[offset] === INTERLEAVED_PREFIX) {
          if (offset + 4 >= chunk.length) break;
          const channel = chunk[offset + 1];
          ...
          if (channel % 2 === 0) {
            this.rtpRingBuffer.push(payload);
            ...
            for (const sub of this.subscribers.values()) {
              if (sub.playing) {
                try {
                  sub.socket.write(Buffer.concat([interleavedHeader(sub.rtpChannel, payload.length), payload]));
                } catch {
                  /* silent on write failures */
                }
              }
            }
          }
    ...
    const onRequest = (
      ...
      if (method === 'SETUP') {
        ...
        sendResponse(cseq, '200 OK', {
          Transport: 'RTP/AVP/TCP;unicast;interleaved=' + (rtpChannel ?? 0) + '-' + ((rtpChannel ?? 0) + 1),
          Session: sessionId,
        });
        return;
      }
